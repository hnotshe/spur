...
//! Shared types for M8 native k0s cluster integration (SPUR owns the k0s lifecycle).
//!
...
mod k0s_config_tests {
    ...
    fn kuberouter_default_generates_no_config() {
        assert!(k0s_controller_config_yaml("kuberouter", "192.0.2.0/24", "198.51.100.0/24", "192.0.2.1", &[]).is_none());
    }
}
