...
//! M8 native k0s cluster controller (leader-gated).
//!
//! Distinct from `crate::cluster` (the raft-backed `ClusterManager` state machine): this drives
//! the SPUR-managed k0s cluster — role selection, IP/CIDR allocation (increment 5b), and the
//! per-node `SlurmAgent` StartClusterComponent/StopClusterComponent fan-out (increment 5c) — all
...
pub async fn run(cluster: Arc<ClusterManager>, raft: Arc<RaftHandle>, net: ClusterNetworking) {
    ...
    loop {
        ...
        // M8 mesh: derive the authoritative full-mesh membership (pubkey + mesh IP + pod /24) from
        ...
        if mesh.nodes.len() >= 2 {
            if mesh.nodes != last_mesh {
                info!(members = mesh.nodes.len(), "k0s full-mesh membership changed");
            }
...
pub(crate) fn provision_assignments(
    ...
    for node in &nodes {
        ...
        let mesh_ip = if is_cp { controller_ip } else { pool.allocate()? };
...
async fn connect_control_plane(cluster: &ClusterManager) -> anyhow::Result<SlurmAgentClient<tonic::transport::Channel>> {
    let cp = cluster
        .k0s_state()
        .control_plane_node
...
/// is unreachable or has no component yet (the spurd agent impl lands in increment 6).
async fn fetch_component_state(cluster: &ClusterManager, node: &str) -> Option<String> {
    ...
    tokio::time::timeout(AGENT_TIMEOUT, fut).await.ok().flatten()
}
...
fn controller_k0s_config(net: &ClusterNetworking, node: &spur_core::node::Node) -> Option<String> {
    ...
    spur_core::k0s::k0s_controller_config_yaml(&net.cni, &net.pod_cidr, &net.service_cidr, api, &sans)
}
...
async fn converge_provisioning(cluster: &ClusterManager, net: &ClusterNetworking) {
    ...
    // Workers: mint a fresh join token from the control-plane agent (M8 step 3), then start with it.
    ...
    for node in &assigned {
        ...
        match mint_worker_token(cluster).await {
            Ok(token) => {
                spawn_start_component(cluster, &node.name, K0sRole::Worker, Some(token), None, node_ip)
            }
            Err(e) => {
...
mod tests {
    ...
    fn carve_pod_cidr_from_16() {
        ...
        assert_eq!(carve_pod_cidr("10.42.0.0/16", 255).unwrap(), "10.42.255.0/24");
    ...
    fn mesh_membership_skips_unmeshed_and_carries_pod_cidr() {
        let nodes = vec![
            ...
            mesh_node("w4", Some("10.44.0.4"), Some(""), Some("198.51.100.4"), None),
