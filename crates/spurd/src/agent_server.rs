...
pub struct AgentService {
    ...
    /// M8: RPC-driven owner of this node's k0s systemd unit.
...
impl SlurmAgent for AgentService {
    ...
    // -- Native cluster component control (M8 increment 6): drive this node's k0s systemd unit. --
    ...
    async fn get_admin_kubeconfig(
        ...
        match self.k0s.admin_kubeconfig().await {
            ...
            Err(e) => Err(Status::internal(format!("k0s kubeconfig admin failed: {e}"))),
        }
    }
    ...
    async fn apply_mesh(
        ...
        let result = tokio::task::spawn_blocking(move || -> anyhow::Result<(bool, usize, String)> {
            ...
            let self_mesh_ip = members
                .iter()
                .find(|n| {
                    self_pubkey.as_deref() == Some(n.public_key.as_str()) || n.hostname == self_host
                })
                .map(|n| n.mesh_ip.clone());
            let Some(self_mesh_ip) = self_mesh_ip else {
                return Ok((false, 0, "this node is not in the pushed mesh membership".to_string()));
            };
            ...
            let (added, pruned) =
                spur_net::mesh::reconcile_mesh(&iface, &self_mesh_ip, &members, &current, false)?;
            Ok((true, added, format!("reconciled mesh: {added} peers, {pruned} pruned")))
        })
        .await
