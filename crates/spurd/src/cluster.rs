...
//! Spurd-owned k0s systemd unit supervisor (M8 "native SPUR <-> k0s integration").
//!
...
impl ClusterSupervisor {
    ...
    async fn k0s_reset(&self) {
        match Command::new(&self.cfg.k0s_binary).arg("reset").output().await {
...
fn parse_execstart(exec: &str) -> anyhow::Result<ClusterConfig> {
    ...
    let node_ip = rest
        .iter()
        .find_map(|a| a.strip_prefix("--kubelet-extra-args=--node-ip=").map(String::from));
    Ok(ClusterConfig {
        role,
...
/// `amd.com/gpu` into pods (M8 increment 7). No-op when the node has no GPUs — containerd rejects
...
/// RPC-driven owner of this node's k0s component (M8 increment 6). The spurctld controller
...
impl K0sAgent {
            ...
            match probe.read_cfg_from_unit().await {
                Ok(cfg) => {
                    ...
                    *self.active.lock().await = Some(ClusterSupervisor::new(cfg));
                    info!(role = role.as_str(), "adopted already-running k0s unit on startup");
                }
                Err(e) => warn!(
    ...
    async fn stop_untracked(&self, reset: bool) {
        ...
        if reset {
            self.probe_supervisor(ClusterRole::Controller).k0s_reset().await;
        }
    }
    ...
    pub async fn create_join_token(&self, role: &str, expiry_seconds: u64) -> anyhow::Result<String> {
...
mod tests {
    ...
    fn parse_execstart_recovers_role_and_paths() {
        ...
        assert_eq!(worker.join_token_file, Some(PathBuf::from("/etc/k0s/token")));
    ...
    async fn derisk_supervise_recovers_unit() {
        ...
        assert!(sup.unit_status_is(&["is-active"], "active").await, "active after reconcile");
        assert!(sup.unit_status_is(&["is-enabled"], "enabled").await, "enabled after reconcile");

        // 2. Out-of-band disable+stop -> reconcile must restore (Restart= does NOT cover this).
        sup.systemctl(&["disable", "--now", &sup.unit_file_name()]).await.expect("disable");
        assert!(!sup.unit_status_is(&["is-active"], "active").await, "stopped after disable --now");
        sup.reconcile_once().await.expect("reconcile after disable");
        assert!(sup.unit_status_is(&["is-active"], "active").await, "reconcile restarts a stopped unit");
        assert!(sup.unit_status_is(&["is-enabled"], "enabled").await, "reconcile re-enables");
        ...
        assert!(sup.unit_path().exists(), "reconcile re-writes a deleted unit file");
        assert!(sup.unit_status_is(&["is-active"], "active").await, "active again after re-write");
    }
}
